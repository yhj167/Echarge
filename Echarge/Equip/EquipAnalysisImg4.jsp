...
<%
	int[] chargingMode = new int[3];//充电模式次数
	...
	for (int i = 0; i < chargings.size(); i++) {
		cc = chargings.get(i);
		String mode = cc.getChargingMode();
		if(mode.equalsIgnoreCase("电量")){
			chargingMode[0]++;
		}
		else if(mode.equalsIgnoreCase("金额")){
			chargingMode[1]++;
		}
		else if(mode.equalsIgnoreCase("时间")){
			chargingMode[2]++;
		}
		else break;
	}
...
<head>
		...
		<title>Echarge 管理中心 - 充电桩充电模式统计图</title>
	...
	<table id="t1" style="table-layout: fixed;" align="center" border="0" cellpadding="0" cellspacing="0" width="97%">
			<tbody>
				...
				<td class="nav" align="left" nowrap="nowrap" width="100%">
					<h1>
						<span class="action-span"></span><span>Echarge 管理中心  - 充电模式统计</span>
					</h1>
				</td>
				</tr>
			</tbody>
	...
	<script type="text/javascript" >
		...
		function getchart(str){
			...
			chart.setDataXML('<?xml version="1.0" encoding="UTF-8" ?>'+
				 '<chart caption="充电桩充电模式统计图" subCaption="(充电模式,选择次数)" bgColor="#ffffff" xAxisName="充电桩号" yAxisName="充电桩号/利用率"'+
				' numberPrefix="" sformatNumberScale="0" sNumberPrefix="$" syncAxisLimits="1" rotateValues="1" showSum="0">'+
				/***生成柱状图*************************************************/
					'<set label="电量模式" value="<%=chargingMode[0]%>"/>'+
					'<set label="金额模式" value="<%=chargingMode[1]%>"/>'+
					'<set label="时间模式" value="<%=chargingMode[2]%>"/>'+
				/************************************************************/
				'</chart>');
