...
<%
	...
	float[][] chargingCost = new float[3][macNum];//某年度充电桩j的充电费用
	for(int x=0; x<3; x++){
		for(int y=0; y<macNum; y++){
			chargingCost[x][y] = 0;//赋初值
		}
	}
	...
	for (int i = 0; i < chargings.size(); i++) {
		...
		chargingCost[year-2010][j-1] += cc.getCost();//year年充电桩j的充电费用增加
	}
%>
...
<head>
		...
		<title>Echarge 管理中心 - 充电桩充电费用统计图</title>
	...
	<table id="t1" style="table-layout: fixed;" align="center" border="0" cellpadding="0" cellspacing="0" width="97%">
			<tbody>
				...
				<td class="nav" align="left" nowrap="nowrap" width="100%">
					<h1>
						<span class="action-span"></span><span>Echarge 管理中心  - 充电费用统计</span>
					</h1>
				</td>
				</tr>
			</tbody>
	...
	<script type="text/javascript" >
		...
		function getchart(str){
			...
			chart.setDataXML('<?xml version="1.0" encoding="UTF-8" ?>'+
				 '<chart caption="充电桩充电费用统计图" subCaption="2010年-2012年" bgColor="#ffffff" xAxisName="年份" yAxisName="充电费用（元）"'+
				...
				'<dataset seriesName="充电桩1">'+
					'<set value="<%=chargingCost[0][0]%>"/>'+
					'<set value="<%=chargingCost[1][0]%>"/>'+
					'<set value="<%=chargingCost[2][0]%>"/>'+
				'</dataset>'+
				'<dataset seriesName="充电桩2" renderAs="line" parentYAxis="S" showValues="1" valuePosition="BELOW">'+
					'<set value="<%=chargingCost[0][1]%>"/>'+
					'<set value="<%=chargingCost[1][1]%>"/>'+
					'<set value="<%=chargingCost[2][1]%>"/>'+
				'</dataset>'+
				'<dataset seriesName="充电桩3" renderAs="line" parentYAxis="S" showValues="1" valuePosition="BELOW">'+
					'<set value="<%=chargingCost[0][2]%>"/>'+
					'<set value="<%=chargingCost[1][2]%>"/>'+
					'<set value="<%=chargingCost[2][2]%>"/>'+
				'</dataset>'+
				'<dataset seriesName="充电桩4" renderAs="line" parentYAxis="S" showValues="1" valuePosition="BELOW">'+
					'<set value="<%=chargingCost[0][3]%>"/>'+
					'<set value="<%=chargingCost[1][3]%>"/>'+
					'<set value="<%=chargingCost[2][3]%>"/>'+
				'</dataset>'+
				'<dataset seriesName="所以充电桩" renderAs="line" parentYAxis="S" showValues="1" valuePosition="BELOW">'+
					'<set value="<%=chargingCost[0][0]+chargingCost[0][1]+chargingCost[0][2]+chargingCost[0][3]%>"/>'+
					'<set value="<%=chargingCost[1][0]+chargingCost[1][1]+chargingCost[1][2]+chargingCost[1][3]%>"/>'+
					'<set value="<%=chargingCost[2][0]+chargingCost[2][1]+chargingCost[2][2]+chargingCost[2][3]%>"/>'+
				'</dataset>'+
				'</chart>');
